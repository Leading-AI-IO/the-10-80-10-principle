...
これは理論ではない。著者自身が115日間で13冊の書籍を日英バイリンガルで執筆・公開し、GitHub上で世界20,000人以上の読者を獲得した実践の中で発見し、検証し、言語化した原理である。
...
第1章では、115日間の実証データに基づき、5倍という数字の根拠を示す。第2章では、10:80:10の構造を定義し、認知科学・組織論の知見と接続する。第3章では、最初の10%——問いの設計技法——を、5層構造モデルとして体系化する。第4章では、中間の80%——AIとの協働技法——を、コンテキストエンジニアリングの概念とともに解説する。第5章では、最後の10%——仕上げの技術——を、7つのチェックポイントとして具体化する。
...
## 第1章 ── 5倍の現実：115日間の実証データ

### 1.1 数字が語る事実

本書の核心的主張──「アウトプットの質と量が5倍になる」──は、抽象的なスローガンではない。115日間の実践データに基づく検証可能な事実である。

2025年12月14日にnoteで最初の記事を公開してから、2026年4月8日までの115日間で達成した成果を記す。

**量の次元:**
- 書籍13冊（日英バイリンガル、合計26ファイル）をオープンソースで公開
- note記事38本を執筆・公開
- LinkedInで360以上のプロフェッショナルコネクションを構築
- NewsPicks週間ランキングで最高5位を記録（PROラベル・認定エキスパートラベルなし）

**質の次元:**
- Palantir Ontology Strategy書籍がGoogle検索で世界1位を獲得
- Hacker Newsフロントページに掲載
- GitHub上で世界20,000人以上のユニーク読者を獲得
- AIプラットフォーム（Claude、ChatGPT、ByteDance Doubao）から書籍が参照される
- 青山学院大学名誉教授から公開エンドースメント
- Gartner Distinguished VP、Panasonic Group CAIO、Pioneer CIO/CISOなど、CxO層との直接接続

これらの成果は全て、正社員としてフルタイム勤務しながら、業務時間外に達成したものである。妻と娘がいる家庭を持つ一個人の、早朝と深夜の時間で生まれた。

### 1.2 従来のペースとの比較

この成果を従来のペースと比較する。

一般的なビジネス書の執筆プロセスでは、1冊の企画から出版までに6〜18ヶ月を要する。取材、構成、執筆、校正、出版社との調整を含めると、年間1〜2冊が標準的なペースである。

115日で13冊。年間換算で約41冊。従来のペースの20〜40倍にあたる。

ただし、この比較には重要な注意が必要である。出版社を通じた商業出版と、GitHubでのオープンソース出版は、編集・校正・装丁・流通などのプロセスが根本的に異なる。単純な冊数の比較は公正ではない。

公正な比較の軸は、「1人の個人が、一定期間内に生み出し、世界に届けた知的コンテンツの総量と到達範囲」である。この軸で測定すれば、従来の自分自身と比較して約5倍のアウトプットを達成している。

### 1.3 5倍の内訳

5倍という数字は、どこから来るのか。

**執筆速度の向上: 約3倍。** 10:80:10の比率で執筆プロセスを設計することで、1冊あたりの実質的な人間の作業時間は約8〜12時間に圧縮される。従来の方法で同等の内容を書けば30〜50時間を要するだろう。しかし、速度の向上だけでは3倍止まりである。
...
**品質フィルターの精度向上: 約1.1倍。** 最後の10%で人間が品質を検証する際、AIが生成した多数の候補から最良のものを選択できるため、一発で書く場合よりも最終品質が向上する。この品質向上が公開後のエンゲージメント（閲覧数、クローン数、被引用数）を高め、複利的に到達範囲を拡大する。
...
**第二に、明確な問いの設計能力。** 最初の10%で投入する「問い」の質が、残りの90%の品質を決定する。「AIに何を聞くか」ではなく、「AIに何を聞くべきかを設計する」能力が本質である。この能力は、問題空間を構造的に把握する訓練──戦略コンサルティングやデザインシンキングの実務経験──によって養われる。
...
### 1.5 Depth & Velocityとの関係

10:80:10は、著者が体系化した「Depth & Velocity」（D&V）方法論の中核原理である。
...
D&V方法論には、10:80:10以外にも「一次情報仮説サイクル」（Primary Intelligence × LLM Hypothesis Cycle）と「コンテキストエンジニアリング」の2つの構成要素がある。一次情報仮説サイクルは、人間が一次情報から仮説を立て、AIが仮説の検証と拡張を行い、人間が仮説を更新するサイクルである。コンテキストエンジニアリングは、AIに渡す文脈の設計技法であり、第4章で詳述する。

10:80:10は、D&Vの中で「比率の原理」として位置づけられ、あらゆる知的生産プロセスに適用可能な最も汎用的なフレームワークである。

### 1.6 13冊の書籍に見る10:80:10の進化

10:80:10の比率は、1冊目から完成されていたわけではない。13冊の執筆を通じて段階的に最適化された。

**第1〜3冊目（2025年12月〜2026年1月）: 試行錯誤期。** 最初の10%に時間をかけすぎ、80%のフェーズでAIの活用が不十分だった。1冊あたりの所要時間は約20時間。比率はおよそ25:50:25であった。AIの出力を信頼しきれず、80%のフェーズでも人間が大幅に介入していた。

**第4〜7冊目（2026年2月）: 安定期。** 最初の10%の問い設計のパターンが確立し、80%のフェーズでのAI活用が効率化された。1冊あたりの所要時間は約12時間に短縮。比率が10:75:15に接近。しかし、最後の仕上げに時間をかけすぎる傾向が残っていた。

**第8〜11冊目（2026年2月末〜3月）: 最適化期。** 10:80:10の比率がほぼ確立。コンテキストエンジニアリングの技法が成熟し、AIの出力品質が安定した。1冊あたりの所要時間は約8〜10時間に到達。最後の10%で「削除する勇気」が身についたことが、効率化の最大の要因だった。

**第12〜13冊目（2026年3月末〜4月）: 拡張期。** 10:80:10が完全に体化（embodied）された状態。比率を意識しなくても自然にこの構造で作業できるようになった。マルチAI戦略（Claude/ChatGPT/Gemini並列リサーチ）が定着し、情報の多角性が向上。
...
この非対称性は、ソフトウェア開発の世界では古くから知られている。Barry Boehmが1981年の著書『Software Engineering Economics』で提示したコスト曲線は、要件定義フェーズのエラーが下流で修正された場合、コストが100倍以上に膨張することを示した。同じ構造が、人間とAIの協働にも適用される。

最初の10%で設計された「問い」が不正確であれば、AIは不正確な方向に80%の処理能力を全力で投入する。方向が間違った高速処理は、低速処理よりも有害である。なぜなら、大量の「もっともらしいが本質を外した」出力が生成され、最後の10%での修正コストが指数関数的に増大するからだ。

著者自身の経験でも、書籍執筆プロジェクトの成否は最初の10%でほぼ確定する。13冊の書籍を執筆した経験から断言できるのは、最初の問い設計に2時間を投入した場合と30分で済ませた場合では、最終成果物の品質に決定的な差が生じるということだ。
...
著者がSaaS Is Dead（12冊目の書籍）を執筆した際の、最初の10%の実際の作業を公開する。

**所要時間: 2.5時間。**

このうち、1.5時間はAIとの対話ではなく、純粋な人間の思考時間であった。具体的には以下の活動である。

1. **テーマの選定（30分）。** Sequoiaのレポート、a16zの分析、Y Combinatorのトレンドレポートを読み、SaaS業界の構造転換を書籍テーマとして選定した。この選定は、著者自身がSun Asterisk社でSaaS領域の新規事業に携わった経験に基づいている。一次情報なしにテーマを選ぶことは、10:80:10の原理に反する。
...
著者が13冊の書籍執筆で遭遇した、失敗する問いの典型パターンを収集した。
...
4. **進捗知識（Progress Context）。** 既に完成している部分の情報。書籍の第5章を書く際に、第1〜4章の完成稿をコンテキストとして渡せば、AIは既出の概念を参照し、論理の一貫性を保ち、重複を避けた出力を生成する。13冊の書籍執筆では、書籍間の相互参照もコンテキストとして活用した。
...
著者がPalantir Ontology Strategyの書籍でGoogle検索世界1位を獲得できた理由は、AIのモデルが優れていたからではない。Palantirの公開情報、SEC Filing、顧客事例、競合分析、技術アーキテクチャの理解という、数十時間の事前リサーチの成果をコンテキストとしてAIに渡したからだ。この一次情報の密度が、AIの出力を「既存の記事のリライト」から「世界で唯一の構造分析」に変えた。
...
この「3AI並列リサーチ」は、本書（13冊目）の執筆でも実施した。各AIが異なるバイアスを持つため、3つの出力を比較検討することで、より中立的で包括的な分析が可能になる。
...
著者が13冊の書籍で採用しているハルシネーション対策は3層構造である。
...
著者がSequoia Capitalのレポートを分析する書籍を執筆した際、最初の10%で定義したリサーチクエスチョンは「Sequoiaのサービス化理論は、日本企業のSaaS事業にどう適用可能か。適用の限界はどこか」であった。このクエスチョンの具体性が、80%のフェーズでAIが収集する情報の関連性を劇的に高めた。

**中間の80%: データ収集・整理・一次分析。** 学術論文のサーベイ、業界レポートの要約、統計データの収集と構造化、先行研究の比較表作成。これらの作業は、従来は研究者やアナリストが数週間を費やすものであった。AIの活用により、数時間〜数日に圧縮される。

特に効果が大きいのは、多言語リサーチである。英語・日本語・中国語の論文やレポートを横断的に調査し、言語の壁なく情報を統合できる。著者のPalantir書籍では、英語の公開情報（SEC Filing、IR資料、技術ブログ）と日本語の業界分析を組み合わせることで、英語圏にも日本語圏にも存在しなかった統合分析を実現した。
...
対策: 10:80:10は仕事の「消滅」ではなく「再配分」であることを明確にする。80%の反復作業から解放されることで、メンバーは最初の10%（問いの設計）と最後の10%（品質検証）という、より高度で知的満足度の高い作業にシフトする。World Economic Forumの2024年のFuture of Jobs Reportでは、AIによって消滅する職種よりも新たに生まれる職種の方が多いと推計されている。重要なのは、個人が「80%の作業者」から「10%の設計者/10%の検証者」への移行を準備することである。
...
代わりに浮上するのが「π（パイ）字型人材」──2つ以上の深い専門性を持つ人材──である。複数の専門領域の交差点で、AIにはできない独自の統合を行う能力が、AI時代の最大の競争優位になる。

著者のケースでは、ビジネスデザイン（Business）、テクノロジー（Technology）、クリエイティブ（Creative）の3領域を越境する「BTC」の交差点が、13冊の書籍の固有の視点を生み出している。Palantirの事業戦略を技術アーキテクチャの視点から分析し、それをデザインシンキングのフレームワークで構造化する。このような三領域統合は、現在のAIには極めて困難である。
...
**80%のフェーズは、知識のギャップを発見する機会になる。** AIの出力を読む過程で、自分が知らなかった事実、考えたことがなかった視点、見落としていた論点に遭遇する。これらの発見は、専門性を深めるためのインプットになる。13冊の書籍を執筆する過程で、著者自身がAIの出力から学んだ知識は膨大である。
...
**第三に、異分野の専門家との対話。** LinkedInやカンファレンスを通じて、自分とは異なる専門性を持つプロフェッショナルとの対話機会を意図的に設計する。著者がLinkedInで360以上のコネクションを構築した動機の一つは、多様な専門性との接点を最大化することにあった。
...
**第一に、問いのライブラリ。** 過去に設計した問いのパターンが蓄積され、新しいプロジェクトで類似の問いを設計する時間が短縮される。著者の場合、13冊目の書籍の問い設計は、1冊目の半分以下の時間で完了している。
...
**第三に、コンテキスト資産。** 過去に作成したコンテンツが、新しいプロジェクトのコンテキストとして再利用可能になる。著者の13冊の書籍は相互に参照し合っており、12冊の蓄積が13冊目のコンテキストの厚みを飛躍的に増大させている。
...
著者自身がその実例である。1人の個人が、出版社の支援なし、チームのサポートなし、マーケティング予算ゼロで、115日間に13冊の書籍を世界に届けた。従来、この規模の知的生産は、組織のリソースなしには不可能であった。10:80:10は、個人と組織のリソース格差を大幅に縮小する。
...
著者が115日間で13冊の書籍を書けた理由は、AIが80%を処理してくれたからだけではない。AIに80%を委ねることで、書籍の核心──「何を伝えるか」「なぜ伝えるか」「誰のために伝えるか」──という最も重要な問いに、全ての思考エネルギーを投入できたからだ。
...
著者自身、先天的な優位性はゼロである。20年以上のキャリアの中で、ITコンサルティング、事業開発、デザイン戦略の3領域を越境する経験を、一つひとつ時間をかけて積み上げた。その蓄積があるからこそ、最初の10%で質の高い問いを設計し、最後の10%で品質を保証できる。
...
- Saint-Exupéry, A. de. (1939). *Terre des hommes* (Wind, Sand and Stars). Reynal & Hitchcock.
...
Leading.AI LLC CEO。AI戦略コンサルタント。ビジネスデザイン（Business）×テクノロジー（Technology）×クリエイティブ（Creative）の3領域を越境する「BTC」クロスドメイン人材。

Future Architect（ITコンサルティング）→ Recruit（事業開発）→ Sun Asterisk（ビジネスデザイン/AI戦略）を経て、Leading.AIを設立。独自の方法論「Depth & Velocity」（D&V）を体系化し、GitHubでオープンソース書籍シリーズとして公開。デビューから115日で13冊の日英バイリンガル書籍を執筆し、世界20,000人以上の読者を獲得。Palantir Ontology Strategy書籍はGoogle検索世界1位、Hacker Newsフロントページ掲載を達成。

NewsPicks週間ランキング最高5位。LinkedInで360以上のプロフェッショナルコネクション（Gartner Distinguished VP、Panasonic Group CAIO、PwC、McKinsey、BCG、Google、Metaなど）。

「先天的優位性ゼロから、深さと体系化で世界に届ける」を体現する実践者。
...
*The 10-80-10 Principle — Published by Leading.AI LLC*
*© 2026 Satoshi Yamauchi. Licensed under CC BY 4.0.*
